...
fc=c*felms;
vb = svboundaries(efnormstd,pivotstd);  % all boundaries
...
title("Boundaries found (including virtual boundaries)")
xlabel("Time(Frame)")
ylabel("Energy")
ax = gca;
ax.FontSize=18;
...
for j=1:length(vb)
    l1=line([vb(j), vb(j)],[max(efnormstd),min(efnormstd)],'Color','blue','LineStyle','--');
end
legend([g1,l1],["Energy","Boundary"])
% subplot(2,1,1)
% g1=plot(y1);
...
g2=plot(efnormstd);
axis([-10 length(efnormstd)+10 min(efnormstd), max(efnormstd)+max(efnormstd)/2])
title("Boundaries found (excluding virtual boundaries)")
xlabel("Time(Frame)")
ylabel("Energy")
ax = gca;
ax.FontSize=18;
